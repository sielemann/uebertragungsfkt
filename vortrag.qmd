...
## Wiederholung 3
 ...
 - Wie können Simulationsaufgaben für lineare und nichtlineare Systeme berechnet werden? Welche Aufgaben gibt es?
   - Lösung für beliebige, zeitvariable Eingänge $u(t)$ ausgehend von $x_0$
      - Analytisch mit *Faltungsgleichung* (nur für lineare Systeme, auch *Bewegungsgleichung*)
      ...
      - Numerisch mit Verfahren für gewöhnliche Differenzialgleichungen
        - Euler, Heun, Runge-Kutta
        - Mehrschrittverfahren etc.
   - Stationäre Initialisierung: Analytische oder numerische Lösung des algebraischen Systems
...
<div style="text-align: center; padding: 50px;">
  <a href="simulation1.html" target="_blank" style="font-size: 0.8em; padding: 20px; background: #039be5; color: white; text-decoration: none; border-radius: 10px;">
    Feder-Masse-Schwinger 1 in neuem Tab öffnen
  </a>
</div>

::: notes
Öffne die Simulation in einem neuen Tab. Danach mit Alt+Tab zurück zur Präsentation!
 - Linearisiertes Beispielmodell, Übergangsverhalten, stationäres Verhalten mit sinusförmigen Eingangssignalen
 - Beim Ändern von Eingangsfrequenz ändern sich Frequenz und Verstärkung von Ausgängen (Verschieben der Parameter: Masse ändert v.a. Amplitude, Federkonstante Amplitude und Phase, Dämpfung vor allem Phase. Verschiebung der Eingangsparameter: Amplitude ändert Amplitude, Frequenz kann beides ändern wenn man an rechten Rand sieht aber nichts besonders eindeutiges).
 - Fehlt/nur erwähnen: Fourieranalyse und Superposition (Kann mit einfachen Signalen und Superpositionsprinzip viele Eingänge darstellen und Ausgänge berechnen). 
:::


## Lernziele heute und Überblick
...
## Lernziele heute und Überblick
 ...
 2. Motivation
 3. **Theorie Teil 1**
    - Ansatz zur Lösung der Faltungsgleichung
    - Exponentielle Eingänge und Berechnung der Ausgänge
    - Übertragungsfunktion und Analyse im Frequenzbereich
    - Superposition
    - Beispiel
 4. Theorie Teil 2
 5. Rückblick
...
::: notes
Öffne die Simulation in einem neuen Tab. Danach mit Alt+Tab zurück zur Präsentation!
 - Übertragungsfunktion aus A, B, C, D berechnen. Aufgabe: Finde stationäre Lösung für u = sin( omega t). Verwende konjugiert komplexes Paar von s. Berechne u1, u2, y1, y2. Berechne y = y1+y2. Plot y_stat, y, u.
...
## Lernziele heute und Überblick
 ...
 3. Theorie Teil 1
 4. **Theorie Teil 2**
    - Konvertierung zwischen Zustandsraum und Übertragungsfunktion
    - Zerlegung von Eingängen in Grundformen
    - Laplace-Transformation und umgekehrte Laplace-Transformation
    - Berechnung von Ausgängen mit Übertragungsfunktionen
    - Darstellung von Übertragungsfunktionen
...
## Konvertierung Übertragungsfunktion zu Zustandsraum 1
 ...
 - Wenn sich die Übertragungsfunktion in Partialbrüchen folgender Form schreiben lässt 
...
::: notes
Fragen
 - Kann jemand erkennen warum die Konvertierung von Übertragungsfunktion zu Zustandsraum nicht eindeutig ist? x_dot=Ax + Bu -> Die Verstärkung von Eingängen im Vergleich zu Zuständen ist nicht eindeutig für stationäre Antwort. Wir wählen die beiden Werte gleich als Wurzeln der Zähler der Partialbrüche (Vergleich zu der vorherigen Folie).
:::
...
## Laplace-Transformation 1
 ...
 - Bedingungen: *Dirichlet-Bedingung*, $\int_{-\infty}^{\infty} \vert u(t) \vert e^{- \delta t} dt < \infty$, d.h., $u(t)$ darf nicht schneller als exponentiell wachsen (nicht $t^t$) und $u(t)$ darf nicht unendlich viele Diskontinuitäten enthalten.

## Laplace-Transformation 2

 - Die inverse Laplace-Transformation transformiert aus dem Frequenzbereich $U(s)$ in den Zeitbereich 
 - Diese Transformation schreiben wir als $\mathcal{L^{-1}}\{U\} = u$
 - Die inverse Transformation ist definiert^[Variable $\gamma$ ist eine reelle Zahl, die größer als der größte Realteil der Nullstellen von $U(s)$ ist.] durch
...
## Zusammenfassung Laplace-Transformation
...
*Vgl. Motivation: Wenn wir uns an den Frequenzbereich halten und Signale darin betrachten lernen, dann können viele Operationen einfacher durchgeführt werden (wie diese Multiplikation, d.h. Algebra statt Analysis)*
...
ax.annotate('', xy=(2.5, 1.2), xytext=(2.5, 0.5),
            arrowprops=dict(arrowstyle='->', lw=1.5))
ax.text(2.8, 0.8, '−', ha='left', va='center', fontsize=10)
...
<!-- Lunze 6.3.3 -->
 - Für technisch interessante Systeme $G(s) = \frac{b_{n-1}s^{n-1} + \dots + b_1 s + b_0}{s^n + a_{n-1}s^{n-1} + \dots + a_1 s + a_0} + d$: Zählergrad $q$ übersteigt Nennergrad $n$ nicht, d.h. $q \leq n$
 - Statische Verstärkung: $G(0) = \frac{b_0}{a_0}$
 - Signale sehr hoher Frequenz werden nicht (wenn Durchgriff $d=0$) oder mit Verstärkung übertragen
    - $\lim_{\omega \to \infty} G(j\omega) = \begin{cases}\frac{b_n}{a_n} + d \quad \text{für } q=n\\d \quad \text{für } q<n\end{cases}$
 - Übertragungsfunktionen sind symmetrisch
    - $\vert G(j\omega) \vert = \vert G(-j\omega) \vert$
    - $\angle G(-j\omega) = -\angle G(j\omega)$
 - Weitere Überlegungen
    - Uns interessiert der Frequenzgang $G(j\omega)$: Verstärkung und Phasenverschiebung
    - Der Einfluss der Dämpfung $\sigma$ in $G(\sigma \pm j\omega)$ braucht nicht detailliert analysiert werden, da dieser wegen Linearität unabhängig aufgeschlagen werden kann 


## Darstellungen der Übertragungsfunktion 1
...
## Darstellungen der Übertragungsfunktion 2

 - *Nyquist-Diagramm* (auch *Ortskurve*)
    - Von einem Zeiger mit Länge $\vert G(j\omega) \vert$ und Winkel $\angle G(j\omega)$ gezeichnete Ortskurve
...
## Beispiele von Übertragungsfunktionen
...
for idx, (name, params) in enumerate(systems.items()):
    ...
    ax3.set_ylim([-60, 20])
    ax3.set_xlabel('ω [rad/s]', fontsize=8)
    ...
    ax4.set_xlabel('ω [rad/s]', fontsize=8)
...
## Nullstellen und Pole
 ...
 - Pole
    ...
    - Verstärkung von Eingängen, die Eigenbewegungen sind, wird unendlich
 - Nullstellen 
    - Bestimmen Eingänge, die vom System nicht übertragen werden (Verstärkung verschwindet in stationärer Lösung)
    - Erzwungene Bewegung (im Gegensatz zu stationärer Lösung) sowie andere Grundeingänge führen in der Regel zu einer Änderung des Ausgangs

*Vgl. Motivation: Insbesondere Pole (d.h. Stabilität) aber auch Nullstellen werden in Regelungstechnik für Strecke und geregelte Strecke entscheidend. Es ist nützlich diese vorhersagen und algebraisch berechnen zu können.*

::: notes
Bezüglich Regelung können wir noch keine Aussage zur Güte der Regelung treffen. Aber ob ein geregelte System stabil ist, ist ein wichtiges Ergebnis. Wir können dies auch konstruktiv nutzen und ableiten, wie wir ein System stabil machen können.
:::
...
<div style="position: absolute; top: 550px; right: 30px;">
  <a href="simulation2.html" target="_blank" style="font-size: 0.8em; padding: 20px; background: #039be5; color: white; text-decoration: none; border-radius: 10px;">
    Feder-Masse-Schwinger 2 in neuem Tab öffnen
  </a>
</div>
...
## Rückblick Lernziele
 ...
 - Zerlegung von Eingängen in Grundformen via Laplace-Transformation (und Superposition)
 - Umrechnungen von Zustandsraum zu Übertragungsfunktion und umgekehrt sind bekannt
 - Darstellung von Übertragungsfunktionen vor allem über Frequenz
 - Nullstellen und Pole beschreiben besondere Eigenschaften des Systems
 - Stabilität kann aus realen Teilen der Pole abgeleitet werden
